...
from afaligner import output_smil, output_json, get_name_from_path, drop_extension, c_FastDTWBD, time_to_str
...
def main():
    ...
    sync_map = align(
        ...
        output_dir='afaligner_data/json/',
        output_format='json',
...
def align(
    ...
    """
    COPY/PASTED FROM afaligner MODULE AND MODIFIED
...
def build_sync_map(
    ...
    synthesizer = Synthesizer(rconf=RuntimeConfiguration("allow_unlisted_languages=True|tts=aws"))
    parse_parameters = {'task_language': 'Takumi', 'is_text_unparsed_id_regex': 'f[0-9]+'}
    ...
    while True:
        if process_next_text:
            ...
            textfile.set_language(Language.JPN)
